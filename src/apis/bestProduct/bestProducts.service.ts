...
// import { IProductCheckSoldout, IProductServiceCreate, IProductServiceDelete, IProductServiceFindOne, IProductServiceUpdate } from "./interfaces/best-products-service.interface";
...
export class BestProductsService{
    //db저장하기위한 세팅
    constructor(
        @InjectRepository(BestProduct)
        private readonly productsRepository: Repository<BestProduct>,
    ...
    //데이터 생성
  async create({createBestProductInput}: IBestProductServiceCreate): Promise<BestProduct> {
        // const { productSaleslocation, productCategoryId,productTags, ...rest } = createProductInput
        const {  productCategoryId,productTags,files, ...rest } = createBestProductInput
   ...
   //데이터 수정
    async update({ productId, updateBestProductInput }: IBestProductServiceUpdate): Promise<BestProduct> {
        ...
        //3. 수정하기
        const { productTags, files,...rest } = updateBestProductInput
        ...
        const result = this.productsRepository.save({
            ...
            // ||
            // ||
            ...bestProduct, //수정 후, 수정되지 않은 다른 결과값까지 모두 객체로 돌려 받고싶을 때!=> 수정한 데이터만 받아오고싶으면 이건 안써줘도 됨!
        //    ...rest,
            ...updateBestProductInput,
            productTags: tags, //id배열이 들어감!
            files: files2
        })
    ...
    checkSoldout({bestProduct}: IBestProductCheckSoldout): void {
        ...
        //     }
         ...
         async delete({ productId }: IBestProductServiceDelete): Promise<boolean> {
            //=>1. 진짜로 삭재하는 방법
        ...
        //** 삭제한 상품 데이터를 다시 복구 */
        async restoreDeleted({ productId }: IBestProductServiceDelete): Promise<boolean> {
